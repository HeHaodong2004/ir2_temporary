...
class Worker:
    def __init__(self, meta_agent_id, policy_net, predictor, global_step, device='cpu', save_image=False):
        ...
        self.missions = {} # For active missions
        self.pending_mission = None # For missions being negotiated
    ...
    def run_episode(self):
        ...
        for t in range(MAX_EPISODE_STEP):
            ...
            # --- NEW MISSION LIFECYCLE LOGIC ---
            
            # 1. NEGOTIATION PHASE (when connected)
            if is_fully_connected:
                self.missions.clear() # Clear all old missions upon regrouping
                try:
                    # Continuously update the pending mission
                    P, r_meet, T_meet, meta = pick_rendezvous_point(self.robots, current_step=t)
                    if P is not None:
                        self.pending_mission = Mission(P, T_meet, r_meet, range(N_AGENTS), start_step=t, meta=meta, pending=True)
                except Exception as e:
                    print(f"[Error] Pending mission update failed: {e}")

            # 2. ACTIVATION PHASE (the moment of disconnection)
            ...
            # 3. EXECUTION PHASE (when disconnected)
                        ...
                        if len(participants_in_group) > 1:
                            ...
                            try:
                                ...
                                if P_new is not None:
                                    new_mission = Mission(P_new, r_new, T_new, new_mission_participants, start_step=t, meta=meta_new)
            ...
            # ... The rest of the loop (Agent State Injection, Decision Making, etc.) remains the same ...
            ...
            # (The rest of the code is identical to the last version you had)
            picks, dists = [], []
            for i, r in enumerate(self.robots):
    ...
    def _update_rdv_paths_per_agent(self):
        # This function now correctly handles both active and pending missions for path calculation
        self._rdv_paths_per_agent = [set() for _ in range(N_AGENTS)]
        missions_to_plan_for = list(self.missions.values())
        if self.pending_mission:
            missions_to_plan_for.append(self.pending_mission)
        ...
        for mission in missions_to_plan_for:
            goal_rc = self._world_to_cell_rc(mission.P, a0.map_info)
            ...
            for aid in mission.participants:
                start_rc = self._world_to_cell_rc(self.robots[aid].location, a0.map_info)
                ...
                if rc_path: self._rdv_paths_per_agent[aid] |= self._cells_to_nodecoord_set(rc_path, a0.map_info)
    ...
    def plot_env(self, step):
        ...
        ax_global.set_aspect('equal', adjustable='box') # FIX: Correct aspect ratio
        ...
        for i, r in enumerate(self.robots):
            pos_cell = self._world_to_cell_rc(r.location, r.map_info)
            if hasattr(r, 'trajectory_x') and r.trajectory_x:
                traj_cells = [self._world_to_cell_rc(np.array([x,y]), r.map_info) for x, y in zip(r.trajectory_x, r.trajectory_y)]
        ...
        # Plot Pending Mission (special style)
        if self.pending_mission:
            p_cell = self._world_to_cell_rc(self.pending_mission.P, self.robots[0].map_info)
            ax_global.plot(p_cell[1], p_cell[0], '+', ms=25, c='yellow', mew=2, zorder=12, alpha=0.8)
            radius = patches.Circle((p_cell[1], p_cell[0]), self.pending_mission.r_meet/CELL_SIZE, fc=(1,1,0,0.05), ec='yellow', ls=':', lw=2, zorder=11, alpha=0.8)
            ax_global.add_patch(radius)

        # Plot Active Missions
        for mission in self.missions.values():
            p_cell = self._world_to_cell_rc(mission.P, self.robots[0].map_info)
            ax_global.plot(p_cell[1], p_cell[0], '*', ms=20, mfc=mission.color, mec='white', mew=2, zorder=12)
            radius = patches.Circle((p_cell[1], p_cell[0]), mission.r_meet/CELL_SIZE, fc=(1,0,0,0.1), ec=mission.color, ls='--', lw=2, zorder=11)
            ax_global.add_patch(radius)
            for aid in mission.participants:
