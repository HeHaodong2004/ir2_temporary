...
class Agent:
    ...
    def update_predict_map(self):
        ...
        for i in range(self.predictor.nsample):
            _, x_inpaint = self.predictor.eval_step(
                x_belief, mask, onehots[i], self.map_info.map.shape,
                letterbox_meta=getattr(self, "_letterbox_meta", None)
            )
    ...
    def pre_process_input(self):
        """
        不缩放原图；仅把 (H_map, W_map) 对齐到更大的 (H_pad, W_pad)，
        这样 netG 在原始分辨率上工作，避免大图“变粗/外扩”的错觉。
        """
        # config['image_shape'] = (W_in, H_in, C)；map.shape = (H_map, W_map)
        W_in, H_in, _ = self.predictor.config['image_shape']
        H_map, W_map = self.map_info.map.shape

        # ---- 选择一个对齐步长（和网络下采样倍数一致即可，8或16通常都行）----
        ALIGN = 16  # 你模型如果是 U-Net/残差带 4 次下采样，16 比较保险

        H_pad = int(np.ceil(H_map / ALIGN) * ALIGN)
        W_pad = int(np.ceil(W_map / ALIGN) * ALIGN)

        pad_top  = (H_pad - H_map) // 2
        pad_left = (W_pad - W_map) // 2
        pad_bot  = H_pad - H_map - pad_top
        pad_right= W_pad - W_map - pad_left

        # ---- belief 与 mask（保持原图，不做缩放）----
        belief_np = self.map_info.map.astype(np.uint8)

        mask_arr = belief_np.copy()
        mask_arr[mask_arr != UNKNOWN] = 0
        mask_arr[mask_arr == UNKNOWN] = FREE    # UNKNOWN->1 后会变成 1.0（ToTensor）

        # 只做 padding（belief 用 UNKNOWN 填；mask 用 0 填即可，反正稍后会被裁掉）
        belief_np = np.pad(belief_np, ((pad_top, pad_bot), (pad_left, pad_right)),
                        mode='constant', constant_values=UNKNOWN)
        mask_np   = np.pad(mask_arr, ((pad_top, pad_bot), (pad_left, pad_right)),
                        mode='constant', constant_values=0)

        # ---- 转 PIL → Tensor（不缩放）----
        x_belief_img = Image.fromarray(belief_np).convert('L')
        mask_img     = Image.fromarray(mask_np).convert('1')
        x_raw_img    = Image.fromarray(self.map_info.map).convert('L')  # 原始大小 (H_map, W_map)

        x_belief = transforms.ToTensor()(x_belief_img).unsqueeze(0).to(self.predictor.device).mul_(2).add_(-1)
        x_raw    = transforms.ToTensor()(x_raw_img  ).unsqueeze(0).to(self.predictor.device).mul_(2).add_(-1)
        mask     = transforms.ToTensor()(mask_img   ).unsqueeze(0).to(self.predictor.device)

        # ---- 记录 meta，供 eval_step 做“去 pad”反变换 ----
        self._letterbox_meta = {
            'H_raw': H_map, 'W_raw': W_map,
            'new_H': H_map, 'new_W': W_map,            # 无缩放
    ...
    def _update_guidepost_path(self):
        """
        用共享 NodeManager 的图，跑一次 Dijkstra；
        候选目标 = utility>0 的节点（等价于 frontier 节点），
        取最近的一个，把“起点->该目标”的最短路标成 mask。
        """
        try:
            nds = self.node_manager.nodes_dict
            # 起点：当前位置贴最近图节点
            start = nds.nearest_neighbors(self.location.tolist(), 1)[0].data.coords
            # 候选：utility>0 的节点（frontier）
            frontier_nodes = []
    ...
    def _assemble_observation_from_shared_graph(self, robot_locations=None, global_intents=None):
        ...
        9) rdv_path(1)              # rendezvous 最短路节点=1
        10) time_left_norm(1)       # 会合紧迫度，所有节点同一常数
        ...
        # ---------- 7) intent_any（单通道，把“通信范围内队友”的未来意图节点标 1） ----------
        def to_key_xy(p):
        ...
        # ---------- 10) guidepost mask（最近 frontier 的最短路） ----------
