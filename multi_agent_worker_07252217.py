...
class MultiAgentWorker:
    def __init__(self, meta_agent_id, policy_net, global_step, device='cpu', save_image=False):
        ...
        # 关键时刻判断阈值
        self.UTILITY_THRESHOLD = 2.5 * UTILITY_RANGE * MIN_UTILITY
    ...
    def run_episode(self):
        ...
        for step in range(MAX_EPISODE_STEP):
            ...
            # 4) 有条件地更新 last_known_intents，并打印 debug 信息
            ...
            for group in groups:
                for i in group:
                    ...
                    for j in group:
                        ...
                        if crit_i or crit_j:
                            print(f"[DEBUG] Step {step}: agent {i} shares intent of agent {j}")
                            self.last_known_intents[i][j] = deepcopy(self.robot_list[j].intent_seq)
                        else:
                            print(f"[DEBUG] Step {step}: agent {i} does NOT share intent of agent {j}")
